...
// Comprueba duplicados
    const querySpec = { query: 'SELECT * FROM c WHERE c.id = @id', parameters: [{ name: '@id', value: nombre }] };
    const { resources: existentes } = await container.items.query(querySpec).fetchAll();
    if (existentes.length > 0) {
      context.res = { status: 409, body: 'El alumno ya existe' };
      return;
    }
  // Crea el alumno
    await container.items.create({ id: nombre, nombre, date: new Date().toISOString(), elecciones: {}, respuestas: [] });
