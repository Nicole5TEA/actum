...
export function ActuaProvider({ children }) {
  ...
  useEffect(() => {
    if (user) {
      setPerfiles(prev => ({
        ...prev,
        [user.name]: { ...(prev[user.name] || { date: user.date }), elecciones } // Conservar fecha si ya existía
      }));
    }
  }, [elecciones, user]);
  ...
  function logout() {
    ...
    setPaso(0);
    setDocente(false);
    localStorage.removeItem('docente_token'); // Eliminar token de docente
    localStorage.removeItem('access_token');  // Eliminar token de acceso general
    setStage('portada'); // Redirigir a la portada
  }
